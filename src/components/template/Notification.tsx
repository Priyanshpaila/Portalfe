            ...
            <Dropdown.Item variant='header'>
                <div className='border-b border-gray-200 dark:border-gray-600 px-4 py-2 flex items-center justify-between'>
                    ...
                    <Tooltip title='Mark all as read'>
                        <Button variant='plain' shape='circle' size='sm' icon={<HiOutlineMailOpen className='text-xl' />} onClick={onMarkAllAsRead} />
                    </Tooltip>
                </div>
            </Dropdown.Item>
                    ...
                    {notificationList.length > 0 &&
                        notificationList.map((item, index) => (
                            <div
                                ...
                                <Badge className='absolute top-4 ltr:right-4 rtl:left-4 mt-1.5' innerClass={`${item.readed ? 'bg-gray-300' : bgTheme} `} />
                            </div>
                        ))}
                    {loading && (
