...
const PageContainer = (props: PageContainerProps) => {
    const { pageContainerType = 'default', children, header, contained = false, extraHeader, 
        // footer = true
     } = props
                    ...
                    {(header || extraHeader) && (
                        <div className={classNames(contained && 'container mx-auto', 'flex items-center justify-between mb-4')}>
                            <div>
                                ...
                                <Suspense fallback={<div></div>}>{header && typeof header !== 'string' && <CustomHeader header={header} />}</Suspense>
                            </div>
