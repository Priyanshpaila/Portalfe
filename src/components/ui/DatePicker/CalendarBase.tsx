...
const CalendarBase = forwardRef<HTMLDivElement, CalendarBaseProps>((props, ref) => {
    ...
    const focusOnNextFocusableDay = (direction: 'down' | 'up' | 'left' | 'right', monthIndex: number, payload: DayKeydownPayload, n = 1) => {
    ...
    const handleDayKeyDown = (monthIndex: number, payload: DayKeydownPayload, event: KeyboardEvent<HTMLButtonElement>) => {
        switch (event.key) {
            case 'ArrowDown': {
