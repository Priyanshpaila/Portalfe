...
const DefaultOption = ({ innerProps, label, selectProps, isSelected, isDisabled, isFocused }: DefaultOptionProps) => {
    ...
    return (
        <div className={classNames('select-option', isSelected && 'selected', isDisabled && 'disabled', isFocused && 'focused')} {...innerProps}>
...
const DefaultDropdownIndicator = ({ selectSize }: { selectSize: string }) => {
    return (
        <div className={classNames('select-dropdown-indicator', selectSize === 'xs' ? '!py-0' : '')}>
            <HiChevronDown />
        </div>
    )
}
...
function _Select<Option, IsMulti extends boolean = false, Group extends GroupBase<Option> = GroupBase<Option>>(
    ...
    if (!isEmpty(form)) {
        ...
        const touchedField = get(touched, field.name) || true
    ...
    const height = selectSize === 'xs' ? '!h-fit !min-h-fit' : `h-${CONTROL_SIZES[selectSize]} min-h-${CONTROL_SIZES[selectSize]}`
    ...
    return (
        <Component<Option, IsMulti, Group>
            ...
            styles={{
                ...
                input: (css) => {
                    return {
                        ...
                        // ...(selectSize === 'xs' ? { paddingBlock: '1px', margin: 0 } : {}),
                    }
                },
                menu: (provided) => ({ ...provided, zIndex: 50 }),
                menuPortal: (provided) => ({ ...provided, zIndex: 50 }),
                // indicatorsContainer: (provided) => ({ ...provided, height: 'fit-content' }),
                ...style,
            }}
            theme={(theme) => ({
                ...theme,
            ...
            components={{
                ...
                DropdownIndicator: () => <DefaultDropdownIndicator selectSize={selectSize} />,
