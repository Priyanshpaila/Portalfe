...
const ConfirmDialog = (props: ConfirmDialogProps) => {
    ...
    return (
        <Dialog contentClassName='pb-0 px-0' closable={!loading && rest.closable} {...rest}>
            ...
            <div className='text-right px-6 py-3 bg-gray-100 dark:bg-gray-700 rounded-bl-lg rounded-br-lg'>
                {showCancelButton && (
                    <Button size='sm' className='ltr:mr-2 rtl:ml-2' disabled={loading} onClick={handleCancel}>
                        {cancelText}
                    </Button>
                )}
                <Button size='sm' variant='solid' loading={loading} color={confirmButtonColor} onClick={handleConfirm}>
                    {confirmText}
                </Button>
            </div>
        </Dialog>
