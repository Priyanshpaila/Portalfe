...
function resolvePoCompanyName(po: any): string {
    const raw = po?.company ?? po?.companyName ?? po?.companyCode ?? po?.shippingAccount?.company ?? ''
...
function isRowForLoggedInUser(row: any, users: UserType[], loggedInUser: any) {
    const rowUserId = typeof row?.user === 'object' && row?.user !== null ? row.user?._id : row?.user

    return users.some((u) => String(u?._id) === String(rowUserId) && String(u?.username) === String(loggedInUser?.username))
}
        ...
        try {
            ...
            setRawData((prev) =>
                prev.map((r) => {
                    ...
                    const nextAuth = (r.authorize || []).map((a: any, i: number) => (i === idx ? { ...a, ...response.data.authorize } : a))
        ...
        return [
            {
                header: '#',
                accessorKey: '',
                cell: ({ cell }: { cell: any }) => <div className="min-w-[36px] text-center">{cell.row.index + 1}</div>,
            },
...

            {
                ...
                cell: ({ row }: { row: any }) => <div className="min-w-[130px] break-words">{row.original.poNumber || '—'}</div>,
            },
            {
                header: 'SAP PO Number',
                accessorKey: 'sapPONumber',
                cell: ({ row }: { row: any }) => <div className="min-w-[130px] break-words">{row.original.sapPONumber || '—'}</div>,
            },
            {
                header: 'Amend No',
                accessorKey: 'amendNumber',
                cell: ({ row }: { row: any }) => <div className="min-w-[80px] text-center">{row.original.amendNumber ?? '—'}</div>,
            },
            {
                header: 'Preview',
                cell: ({ row }: { row: any }) => (
                    <div className="min-w-[90px]">
                        <POPrintComponent po={row.original} />
                    </div>
                ),
            },
            {
            ...
            },
            {
                header: 'PO Date',
                accessorKey: 'poDate',
                cell: ({ row }: { row: any }) => <div className="min-w-[110px]">{row.original.poDate || '—'}</div>,
            },
            ...
            {
                header: 'Last Authorized By',
                accessorKey: 'authorizedBy',
                cell: ({ row }: { row: any }) => <div className="min-w-[180px] break-words">{row.original.authorizedBy || '—'}</div>,
            },
            {
                header: 'Last Authorized At',
                accessorKey: 'authorizedAt',
                cell: ({ row }: { row: any }) => <div className="min-w-[150px] break-words">{row.original.authorizedAt || '—'}</div>,
            },
            {
                header: 'Company',
                accessorKey: 'company',
                cell: ({ row }: { row: any }) => <div className="min-w-[180px] break-words">{row.original.company || '—'}</div>,
            },
            {
                header: 'Purchase Type',
                accessorKey: 'purchaseType',
                cell: ({ row }: { row: any }) => <div className="min-w-[120px] break-words capitalize">{row.original.purchaseType || '—'}</div>,
            },
            {
                header: 'Ref. Document Type',
                accessorKey: 'refDocumentType',
                cell: ({ row }: { row: any }) => <div className="min-w-[150px] break-words">{row.original.refDocumentType || '—'}</div>,
            },
            {
                header: 'Ref. Document No',
                accessorKey: 'refDocumentNumber',
                cell: ({ row }: { row: any }) => <div className="min-w-[140px] break-words">{row.original.refDocumentNumber || '—'}</div>,
            },
            {
                header: 'Vendor Name',
                accessorKey: 'vendorName',
                cell: ({ row }: { row: any }) => <div className="min-w-[180px] break-words">{row.original.vendorName || '—'}</div>,
            },
            {
                header: 'Vendor Location',
                accessorKey: 'vendorLocation',
                cell: ({ row }: { row: any }) => <div className="min-w-[180px] break-words">{row.original.vendorLocation || '—'}</div>,
            },
            {
                header: 'Freight Type',
                accessorKey: 'freightType',
                cell: ({ row }: { row: any }) => <div className="min-w-[120px] break-words">{row.original.freightType || '—'}</div>,
            },
            {
                header: 'Scheduled Date',
                accessorKey: 'scheduledDate',
                cell: ({ row }: { row: any }) => <div className="min-w-[120px]">{row.original.scheduledDate || '—'}</div>,
            },
            {
                header: 'Contact Person',
                accessorKey: 'contactPersonName',
                cell: ({ row }: { row: any }) => <div className="min-w-[150px] break-words">{row.original.contactPersonName || '—'}</div>,
            },
            {
                header: 'Serial No',
                accessorKey: 'serialNumber',
                cell: ({ row }: { row: any }) => <div className="min-w-[120px] break-words">{row.original.serialNumber || '—'}</div>,
            },
            {
                header: 'Validity Date',
                accessorKey: 'validityDate',
                cell: ({ row }: { row: any }) => <div className="min-w-[120px]">{row.original.validityDate || '—'}</div>,
            },
            {
                header: 'Department Name',
                accessorKey: 'departmentName',
                cell: ({ row }: { row: any }) => <div className="min-w-[160px] break-words">{row.original.departmentName || '—'}</div>,
            },
            {
                header: 'Remarks',
                accessorKey: 'remarks',
                cell: ({ row }: { row: any }) => (
                    <div className="min-w-[180px] max-w-[320px] break-words whitespace-normal">{row.original.remarks || '—'}</div>
                ),
            },
        ]
        ...
        <div className="w-full min-w-0">
            ...
            >
                <div className="space-y-2">
                    ...
                    <div>
                        <p className="mb-1 text-xs font-semibold">Comment {approvalDialog?.status === 2 ? '(required for reject)' : '(optional)'}</p>
                        <Input
                            textArea
