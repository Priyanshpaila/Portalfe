...
const particularsTotal = [
  { label: 'Basic After Disc (Total)', value: 'basicAfterDiscount' },
  {
    ...
    getValue: (vendor: CSVendor) =>
      vendor.charges ? `${vendor.charges.otherCharges.amount} (${vendor.charges.otherCharges.gstRate}%)` : 0,
  },
  {
    label: 'Packaging & Forwarding',
    value: 'packagingForwarding',
    getValue: (vendor: CSVendor) =>
      vendor.charges ? `${vendor.charges.packagingForwarding.amount} (${vendor.charges.packagingForwarding.gstRate}%)` : 0,
  },
  { label: 'GST', value: 'gst' },
    ...
    <div className="mb-3">
      {isItemWise && (
        <div className="mb-2 text-[11px] text-slate-600">
          <span className="font-semibold text-slate-700">Note:</span> “Totals Best” badges are based on overall totals.
          Item-wise best vendors are shown on each item card. Counts below show wins across items.
        </div>
      )}
      ...
      {open && (
        <div className="px-4 pb-4">
          ...
          <div className="mt-2 space-y-1">
            {particulars.map((p) => (
              <div key={`p-${item._id}-${p.value}`} className="flex border border-slate-200 rounded-xl overflow-hidden bg-white">
...
export function ComparativeStatementComponent({ viewOnly, rfqNumber, csNumber }: { viewOnly: boolean; rfqNumber?: string; csNumber?: string }) {
  ...
  const [vendorModalState, setVendorModalState] = useState<
    Pick<CSType['items'][0], 'indentNumber' | 'itemCode' | 'itemDescription' | 'qty'> | null
  >(null)
          ...
          <TabContent value={tabs[0]}>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
              ...
              <div className="w-[340px] max-w-full">
                <Input size="xs" placeholder="Search item (desc / code / indent)…" value={itemQuery} onChange={(e: any) => setItemQuery(e.target.value)} />
              </div>
            </div>
            ...
            {viewMode === 'readable' && (
              <div className="space-y-3">
                ...
                <TotalsAccordionCard vendors={((csSheet as any)?.vendors || []) as any} leastValues={(csSheet as any)?.leastValues} expand={expandAll} />
              ...
              <Table compact className="small-table" containerClassName="overflow-auto whitespace-nowrap w-full relative">
                ...
                <TBody>
                  ...
                  <Tr>
                    ...
                    {(csSheet as any)?.vendors?.map((_v: any, idx: number) => (
                      <Td key={'qr:' + idx} className="text-center"></Td>
                    ))}
                    <Td />
                    <Td />
                  </Tr>
                        ...
                        <Tr key={`particular:${i._id}:${p.value}`}>
                          ...
                          {i.vendors?.map((v: any, vIdx: number) => (
                            <Td
                              ...
                              className={classNames(
                                'text-center !pr-1',
                                i?.leastValues?.[p.value] && i?.leastValues?.[p.value]?.vendorCode === vIdx + 1 ? 'bg-yellow-400/40 text-black' : null,
                              )}
                            >
                              {v[p.value]}
                              {p?.extra}
                      ...
                      {(csSheet as any)?.vendors?.map((v: any, vIdx: number) => (
                        <Td
                          ...
                          className={classNames(
                            'text-center !pr-1',
                            idx === 0 ? 'border-t border-t-slate-500' : null,
                            (csSheet as any)?.leastValues?.[p.value] && (csSheet as any)?.leastValues?.[p.value]?.vendorCode === vIdx + 1
                  ...
                  <Tr>
                    ...
                    <Td>Freight Type</Td>
                    {(csSheet as any)?.vendors?.map((v: any, idx: number) => (
                      <Td key={'freightType' + idx}>{v.freightType || null}</Td>
                    ))}
                    <Td colSpan={2} />
                  </Tr>
                  ...
                  <Tr>
                    ...
                    <Td>Remarks</Td>
                    {(csSheet as any)?.vendors?.map((v: any, idx: number) => (
                      <Td key={'remarks' + idx}>{v.remarks || null}</Td>
                    ))}
                    <Td colSpan={2} />
                  </Tr>
                  ...
                  {termsConditionsOptions.map((tc) => (
                    <Tr key={tc.value}>
                      ...
                      <Td>{tc.label}</Td>
                      {(csSheet as any)?.vendors?.map((v: any, idx: number) => (
                        <Td key={'tc' + idx}>{v.termsConditions?.[tc.value]}</Td>
                      ))}
                      <Td colSpan={2} />
                    </Tr>
                  ))}
                  ...
                  <TBody>
                    {selection?.map((s: any, idx: number) => (
                      <Tr key={`selection-${idx}`}>
                        ...
                        <Td>
                          {isEditable && (
                            <button type="button" className="text-red-600" onClick={() => setSelection((prev) => prev.slice(0, idx).concat(prev.slice(idx + 1)))}>
                              REMOVE
                            </button>
                          )}
                        </Td>
                    ...
                    {(csSheet as any)?.vendors
                      ...
                      ?.map((v: any, idx: number) => (
                        ...
                        >
                          <Td>{idx + 1}</Td>
                          <Td>
                            <input disabled={!isEditable} type="radio" checked={(selection as any)?.[0]?.vendorCode === v.vendorCode} onChange={() => null} />
                          </Td>
                          <Td>{v.name}</Td>
    ...
    <div className="text-xs">
      ...
      <div className="mb-4 flex justify-between">
        <div className="flex gap-2 items-center">
          ...
          <span className={classNames('px-2 font-semibold text-white h-full flex items-center rounded-md', csSheet?.status === 1 ? 'bg-green-600' : 'bg-slate-600')}>
                ...
                <Tr key={`vendor-2-${idx}`}>
                  ...
                  <Td>
                    <input
                      ...
                      type="checkbox"
                      checked={
                        selection?.find((i: any) => (csType === csTypes[0].value ? i.itemCode === vendorModalState?.itemCode : true) && i.vendorCode === v.vendorCode)
                          ?.checked
                      }
                      onChange={(e) =>
                    ...
                    <Input
                      ...
                      onChange={(e: any) =>
                        usedQty - vendorQty + +e.target.value > +vendorModalState?.qty || e.target.value.split('.')?.[1]?.length > 3
                          ? null
                          : setSelection((prev: any[]) => prev.map((i: any) => (i.vendorCode === v.vendorCode ? { ...i, qty: e.target.value } : i)))
                      }
                    />
                  </Td>
                </Tr>
