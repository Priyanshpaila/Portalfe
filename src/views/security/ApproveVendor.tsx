...
import { MdCheckCircle, MdContentCopy, MdOpenInNew, MdRefresh, MdSearch, MdVisibility } from 'react-icons/md'
...
/** ✅ Update this to your preferred GST verification portal */
const GST_VERIFY_BASE_URL = 'https://services.gst.gov.in/services/searchtp'

function cleanGstin(gstin?: string) {
    return String(gstin ?? '')
        .trim()
        .toUpperCase()
        .replace(/\s+/g, '')
...
async function openVerifyGstin(gstin?: string) {
    const g = cleanGstin(gstin)
    if (!g) return

    // 1) Copy GSTIN (so user can paste if portal ignores query param)
    try {
        await navigator.clipboard.writeText(g)
        // optional toast: "GSTIN copied"
    } catch {
        // ignore (clipboard may be blocked on some browsers)
    }

    // 2) Try opening with query param (may or may not auto-fill depending on portal behavior)
    const url = `${GST_VERIFY_BASE_URL}`

    // Use noopener for security
    window.open(url, '_blank', 'noopener,noreferrer')
}

function GstCell({ gstin }: { gstin?: string }) {
    const g = cleanGstin(gstin)
            ...
            <Button
                ...
                onClick={async () => {
                    try {
                        ...
                        showAlert('GSTIN copied')
                    } catch (e: any) {
                        showError(e?.message || 'Failed to copy GSTIN')
                    }
                }}>
                Copy
            </Button>

            <Button type='button' size='xs' variant='plain' icon={<MdOpenInNew />} onClick={() => openVerifyGstin(g)}>
                Verify
            </Button>
        </div>
    )
                                                    ...
                                                    <Td className='text-right'>
                                                        <div className='flex justify-end gap-2'>
                                                            ...
                                                            <Button
                                                                size='xs'
                                                                variant='solid'
                                                                disabled={!!flags.approving}
                                                                icon={
                                                                    flags.approving && confirmVendor?._id === v._id ? <Spinner size={14} /> : <MdCheckCircle />
                                                                }
                                                                onClick={() => askApprove(v)}>
                                                                {flags.approving && confirmVendor?._id === v._id ? 'Approving...' : 'Approve'}
                                                            </Button>
                                                        </div>
                                                    </Td>
                                                </Tr>
                ...
                <div className='mt-5 flex items-center justify-end gap-2 border-t pt-4'>
                    ...
                    {selected ? (
                        <Button
                            ...
                            variant='solid'
                            disabled={!!flags.approving}
                            icon={flags.approving && confirmVendor?._id === selected._id ? <Spinner size={16} /> : <MdCheckCircle />}
                            onClick={() => askApprove(selected)}>
                            {flags.approving && confirmVendor?._id === selected._id ? 'Approving...' : 'Approve'}
                        </Button>
                    ) : null}
                </div>
            </Dialog>
